...
import logging
import numpy as np
# Try to import xgboost, fall back to our mock implementation if not available
try:
    import xgboost as xgb
except ImportError:
    try:
        import mock_xgboost as xgb
        logging.warning("Using mock_xgboost implementation")
    except ImportError:
        logging.error("Neither xgboost nor mock_xgboost could be imported")
        # Create a very basic mock to avoid crashing
        class MockXGB:
            class XGBClassifier:
                def __init__(self, **kwargs): pass
                def predict(self, X): return np.ones(X.shape[0]) * 0.5
                def predict_proba(self, X): return np.ones((X.shape[0], 2)) * 0.5
        xgb = MockXGB()
...
def evaluate_startup(doc: dict, xgb_model) -> Dict[str, Any]:
    ...
    return {
        ...
        "flashdna_score": camp_scores["flashdna_score"],  # Use CAMP scoring as primary
...
def evaluate_startup_camp(doc: dict, xgb_model) -> Dict[str, Any]:
    ...
    """
    # Calculate individual CAMP component scores
    ...
    # Calculate base CAMP score with proper weightings
    base_camp_score = (
        capital_score * 0.30 +   # Capital Efficiency (30%)
        market_score * 0.25 +    # Market Dynamics (25%)
        advantage_score * 0.20 + # Advantage Moat (20%)
        people_score * 0.25      # People & Performance (25%)
    )
    
    # Apply intangible modifier (±20% adjustment)
    intangible = doc.get("intangible", 50.0)
    intangible_modifier = ((intangible - 50) / 50) * 0.2  # -0.2 to +0.2 range
    intangible_adjustment = base_camp_score * intangible_modifier
    
    # Calculate final score with intangible modifier
    final_score = base_camp_score + intangible_adjustment
    ...
    return {
        ...
        "base_camp_score": base_camp_score,
        "intangible_value": intangible,
        "intangible_modifier": intangible_modifier,
...
def calculate_advantage_score(doc: dict) -> float:
    ...
    try:
        ...
        # Base score calculation (0-100 scale)
        score = 40.0  # Start slightly below neutral (harder to have moats)
...
def train_model(X: np.ndarray, y: np.ndarray) -> xgb.XGBClassifier:
    """
    train XGBoost with default XGB_PARAMS
    """
    ...
    # Make a copy of params and remove use_label_encoder if present (compatibility with newer XGBoost)
