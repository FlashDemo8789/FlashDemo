...
def scenario_runway(
    ...
    This function models a startup's cash flow over time based on:
    ...
    Args:
        burn_rate (float): Monthly burn rate (expenses)
        current_cash (float): Current cash balance
        monthly_revenue (float, optional): Starting monthly revenue. Defaults to 0.0.
        rev_growth (float, optional): Monthly revenue growth rate as decimal. Defaults to 0.0.
        cost_growth (float, optional): Monthly cost growth rate as decimal. Defaults to 0.0.
        months (int, optional): Number of months to forecast. Defaults to 24.
    
    Returns:
        Tuple[int, float, List[float]]: 
            - runway: Number of months until cash runs out (-1 if doesn't run out)
            - ending_cash: Cash balance at the end of forecast period
            - cash_flow: List of cash balances for each month
    """
    flow = []
    runway = -1  # Default, will be set if cash goes negative
    cash = current_cash
    rev = monthly_revenue
    br = burn_rate

    for m in range(1, months + 1):
        net = rev - br  # Net cash flow for the month
        cash += net
        flow.append(cash)
        ...
        # Track when cash first goes negative
        if cash < 0 and runway < 0:
            runway = m

    # If cash never goes negative during the forecast period
    if runway < 0:
        runway = 9999  # Effectively infinite within forecast horizon
