...
class StartupAssessmentModel:
    """
    example advanced model that processes doc across multiple categories
    """
    ...
    def _generate_analysis(self, cat_scores: Dict[str,float], feats: Dict[str,Any], data: Dict[str,Any])-> Dict[str,Any]:
        ...
        if ov>=75:
            analysis["overall"]= "Excellent => Flash DNA top-tier"
        elif ov>=60:
            analysis["overall"]= "Strong => Flash DNA mostly good metrics"
        elif ov>=45:
            analysis["overall"]= "Moderate => Flash DNA improvement needed"
        else:
            analysis["overall"]= "Underperforming => Flash DNA major improvements needed"
    ...
    def _analyze_financial(self, sc, feats, data)-> str:
        if sc>=75:
            return "Excellent financial health Flash DNA"
        elif sc>=60:
            return "Good finances => Flash DNA sustainable"
        elif sc>=45:
            if feats.get('ltv_cac_ratio',0)<0.5:
                return "Mediocre Flash DNA finances => borderline unit economics"
            else:
                return "Ok Flash DNA finances => some improvement needed"
        else:
            run= feats.get('runway_months',0)
            if run<6:
                return "Critical => Flash DNA short runway => immediate attention"
            else:
                return "Weak Flash DNA finances => risk of unsustainable burn"

    def _analyze_growth(self, sc, feats, data)-> str:
        if sc>=75:
            return "Outstanding Flash DNA growth => strong traction"
        elif sc>=60:
            return "Healthy Flash DNA growth => balanced acquisition & retention"
        elif sc>=45:
            c= feats.get('churn_rate',0)
            if c>0.1:
                return "Moderate Flash DNA growth => churn hamper scaling"
            else:
                return "Moderate can improve"
        else:
            return "Struggling needs fundamental acquisition/retention improvements"

    def _analyze_team(self, sc, feats, data)-> str:
        if sc>=75:
            return "Highly experienced Flash DNA team => strong execution"
        elif sc>=60:
            return "Good Flash DNA team => some gaps"
        elif sc>=45:
            return "Moderate Flash DNA team => partial domain or leadership shortfalls"
        else:
            return "Weak Flash DNA team => lacking leadership or domain experience"

    def _analyze_product(self, sc, feats, data)-> str:
        if sc>=75:
            return "Mature Flash DNA product => strong NPS & adoption => high user value"
        elif sc>=60:
            return "Good Flash DNA product => some areas for improvement"
        elif sc>=45:
            return "Average Flash DNA product => lacks strong user love or differentiation"
        else:
            return "Underdeveloped Flash DNA product => significant improvements needed"

    def _analyze_market(self, sc, feats, data)-> str:
        if sc>=75:
            return "Large Flash DNA market => strong advantage & favorable conditions"
        elif sc>=60:
            return "Good Flash DNA market => decent size/growth"
        elif sc>=45:
            return "Middling smaller or slower growth => watch competition"
        else:
            return "Unfavorable too small or saturated"
    ...
    def _generate_recommendations(self, cat_scores, feats, data)-> List[str]:
        ...
        if cat_scores.get('financial',0)<50:
            recs.append("Revisit Flash DNA financial strategy => reduce burn or improve revenue")
        if cat_scores.get('growth',0)<50:
            recs.append("Improve user acquisition & retention => test new channels")
        if cat_scores.get('team',0)<50:
            recs.append("Strengthen team => hire missing domain skill")
        if cat_scores.get('product',0)<50:
            recs.append("Enhance product => gather user feedback & fix top friction")
        if cat_scores.get('market',0)<50:
            recs.append("Consider Flash DNA pivot => current market might be tough")
        if not recs:
            recs.append("Maintain Flash DNA momentum => scale while sustaining quality")
        return recs
