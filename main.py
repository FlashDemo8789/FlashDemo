...
def run_train():
    logger.info("Starting HPC synergy BFS–free training => model_xgb.pkl")
    train_model_xgb()
    logger.info("Training done => HPC synergy BFS–free")

def run_hpc():
    logger.info("Running HPC synergy BFS–free simulations => churn/referral range")
    try:
...
def run_api_server(host="0.0.0.0", port=5000):
    logger.info(f"Starting HPC synergy BFS–free API on {host}:{port}")
...
def run_streamlit():
    logger.info("Starting HPC synergy BFS–free Streamlit => analysis_flow.py")
    sys.argv = [
        "streamlit", 
        "run", 
        "analysis_flow.py",
...
def main():
    parser= argparse.ArgumentParser(description="FlashDNA Infinity HPC synergy BFS–free CLI")
